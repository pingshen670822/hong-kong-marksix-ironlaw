...
def build_reports(a,history):
    ...
    version={"updated_at":datetime.now().isoformat(timespec="seconds"),"latest_period":a["latest_draw"]["period"],"hash":hashlib.sha256(analysis.encode()).hexdigest()[:16]}
...
<p class="notice">{e(a['notice'])}</p></main><footer>資料基準 {a['latest_draw']['date']}・目標 {a['target_date']}・核心 {a['engine']}</footer><script src="app.js"></script></body></html>'''
    ...
    confidence_html=f'''<article class="hero-recommend"><span class="confidence-label">{e(conf['label'])}</span><h2>強烈推薦 {balls([conf['number']])}</h2><p>校準機率 {conf['calibrated_probability']*100:.3f}%・公平基準 {conf['fair_probability']*100:.3f}%・相對提升 {conf['relative_lift_pct']:.2f}%</p><p>模型前9支持 {conf['model_top9_support']}/{len(bt['main']['names'])}・加權共識 {conf['weighted_support_pct']:.2f}%</p><small>{e(conf['warning'])}</small></article><h2>多項邏輯驗證</h2>{table(['檢查項目','結果'],check_rows)}<h2>清楚分層推薦</h2><article class="pack tier-a"><h3>A級・唯一最強</h3>{balls(tiers['A_唯一最強'])}</article><article class="pack tier-b"><h3>B級・高信心前三</h3>{balls(tiers['B_高信心前三'])}</article><article class="pack"><h3>C級・核心前九</h3>{balls(tiers['C_核心前九'])}</article><article class="pack"><h3>D級・次高防守</h3>{balls(tiers['D_次高防守'])}</article><article class="pack low"><h3>E級・低機率暫避</h3>{balls(tiers['E_低機率暫避'],'avoid')}</article>'''
